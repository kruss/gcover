...
class UnitTest
	...
	def moveGcovFiles
	    begin 
	      subfolder = $AppOptions[:eclipse] ? "*/" : ""
	      moveFiles("#{@projectFolder}/#{subfolder}*.o.log", @outputFolder)
	      moveFiles("#{@projectFolder}/#{subfolder}*###*.gcov", "#{@outputFolder}/gcov/extern") # first move external gcov-files
	      moveFiles("#{@projectFolder}/#{subfolder}*.gcov", "#{@outputFolder}/gcov")            # now move the real ones
	    rescue => error
	      @logger.dump error
	    end
	end
